...
def dmplug(model, scheduler, logdir, img='00000', eta=0, lr=1e-2,lrk=1e-1, kernel='motion', dataset='celeba',img_model_config=None,task_config=None,device='cuda'):
    dtype = torch.float32
    ...
    for iterator in range(epochs):
        ...
        loss = criterion(operator.forward(output, out_k), y_n)
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    task_config = 'configs/tasks/{}_deblur_config.yaml'.format(opt.kernel)
    ...
    logdir = os.path.join(opt.logdir, 'BID', opt.kernel, opt.dataset, img)
    os.makedirs(logdir,exist_ok=True)
    # DMPlug
    dmplug(model, scheduler, logdir, img=img, eta=opt.eta, lr=opt.lr, lrk=opt.lrk, kernel=opt.kernel, dataset=opt.dataset, img_model_config=img_model_config, task_config = task_config, device=device)