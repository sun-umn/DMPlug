...
def clear_color2(x: torch.Tensor) -> np.ndarray:
    if torch.is_complex(x):
        x = torch.abs(x)
...
def perform_tilt(x, tilt, image_size: int, device):
    ...
    grid_x, grid_y = torch.tensor(np.meshgrid(range(1, image_size + 1), range(1, image_size + 1)), device=device)
    # Avoid in-place operations by creating new tensors
    tilt_act = torch.stack([
        tilt_act[:, :, 0] + grid_x,
        tilt_act[:, :, 1] + grid_y,
    ], dim=-1)
    tilt_act = torch.stack([
        normalize_axis(tilt_act[:, :, 0], image_size),
        normalize_axis(tilt_act[:, :, 1], image_size),
    ], dim=-1)
